...
{
    public partial class ProfilGuncelleme : System.Web.UI.Page
    {
        ...
        private void PuanDurumunuYukle()
        {
            ...
            string query = "SELECT Puan FROM Kullanicilar WHERE ID = @KullaniciID";
        ...
        private void ResimYukle()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection("Data Source=LAPTOP-KH2PNG3N\\SQLEXPRESS02;Initial Catalog=EtkinlikYonetimSitesi;Integrated Security=True"))
                {
                    ...
                    string query = "SELECT Resim FROM Kullanicilar WHERE ID = @KullaniciID";
        ...
        {
            VeriTabaniBaglantisi.BaglantiKontrolu();
            string query = "SELECT KullaniciAdi, Sifre, Eposta, Ad, Soyad, DogumTarihi, Cinsiyet,  Konum, TelefonNumarası, Resim, Puan FROM Kullanicilar WHERE ID = @ID";
            using (SqlCommand cmd = new SqlCommand(query, VeriTabaniBaglantisi.baglanti))
            {
        ...
        {    
            ...
            try
            {
                ...
                string query = "UPDATE Kullanicilar SET Sifre = @Sifre, Eposta = @Eposta, Ad = @Ad, Soyad = @Soyad, DogumTarihi = @DogumTarihi, Cinsiyet = @Cinsiyet, Konum=@Konum, TelefonNumarası = @TelefonNumarasi, Resim = @Resim WHERE ID = @ID";
                using (SqlCommand cmd = new SqlCommand(query, VeriTabaniBaglantisi.baglanti))
                {
                    cmd.Parameters.AddWithValue("@ID", KullaniciID);
            ...
            "setTimeout(function(){ window.location = 'KullaniciSayfasi.aspx'; }, 2000);", true);
            }
            catch (Exception ex)
            {
